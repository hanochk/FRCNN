...
def _compute_targets(ex_rois, gt_rois, labels):
    ...
    if cfg.TRAIN.BBOX_NORMALIZE_TARGETS_PRECOMPUTED:
        ...
        targets = ((targets - np.array(cfg.TRAIN.BBOX_NORMALIZE_MEANS[:4]))
                   / np.array(cfg.TRAIN.BBOX_NORMALIZE_STDS[:4]))
    return np.hstack(
...
def _sample_rois(all_rois, gt_boxes, fg_rois_per_image, rois_per_image, num_classes):
    ...
    gt_assignment = overlaps.argmax(axis=1)  # which ground truth box matches every roi
