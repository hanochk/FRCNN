...
def proposal_layer(rpn_cls_prob_reshape, rpn_bbox_pred, im_info, cfg_key, _feat_stride=16, anchor_scales=[8, 16, 32],
    ...
    _anchors = generate_anchors(base_size=_feat_stride, scales=np.array(anchor_scales), ratios=np.array(anchor_ratios))
    ...
    shifts = np.vstack((shift_x.ravel(), shift_y.ravel(),
                        shift_x.ravel(), shift_y.ravel())).transpose()  # shape is (H, W, 4).reshape((H*W,4))
